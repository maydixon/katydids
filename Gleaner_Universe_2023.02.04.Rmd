...
#bootstrapped means -  one way to make individual plots
```{r}
...
unranked_batplots <- plot_bats(d = g_u, ranked = FALSE)
unranked_batplots

ranked_batplots <- plot_bats(d = g_u, ranked = TRUE)
ranked_batplots


names(myplots) <- unique(g_u$Species)
names(bootsummary) <- unique(g_u$Species)
ggpubr::ggarrange(plotlist = unranked_batplots[1:8], ncol = 1, common.legend = TRUE)
...
#minimum values for plotting (could also do by bat, not observations per playback)
Boot_sums <- Boot_sums %>%
      mutate(low = ifelse(n.obs>=7, low,NA )) %>% #min value for plotting 95% bats
      mutate(high = ifelse(n.obs>=7, high, NA )) %>%
      mutate(mean = ifelse(n.obs>=2, mean, NA )) #min value for plotting boot means
...
Species_levels <- levels(fct_reorder( g_u$Species_full, -g_u$Response, .fun='mean'))
mutate(Species_full = fct_reorder( Species_full, -Response, .fun='mean'))

mutate(Species_full = factor( Species_full, levels = Species_levels))
...
plot_bats <- function(d=d, ranked= FALSE) {
      
    
Species_levels <- levels(fct_reorder( d$Species_full, -d$Response, .fun='mean'))
      
   #return here: trying to make this refer to the current facet group
...
# subset(Boot_sums, Boot_sums$Species_full == string)
  #how to return first value of each group? 
...
#Why don't these reorder facets? infuriating
     ...
     #mutate(Species_full = fct_reorder( Species_full, -Response, .fun='mean'))
        #mpg$class2 <- reorder(mpg$class, mpg$displ)
                  ...
                  # mutate(Species_full = factor(Species_full, levels = levels(fct_reorder( Species_full, -Response, .fun='mean')))
 ...
 if(ranked){
    plot <-   
     ...
     plot <- plot +
          ...
          xlab("Playback") +
          ylab("Response scores") +
          ggtitle(paste(unique(d$Species_full)," (n = ", length(unique(d$ID)), ")", sep ="") )  
      ...
      # #make pdfs of plots (85, 129, or 177 mm width to fit in 1-3 columns dimensions)  
       # ggsave("testplot.jpg",  width = 177, height =800, units = "mm")
                 plot = plot +   ggsave(filename = paste(ifelse(ranked,"ranked", "unranked"), "all bats", "responses_to_playbacks", ".jpg", sep = ""), width = 177, height =800, units = "mm" )
...
unranked_plot <- plot_bats(d = g_u, ranked = FALSE)
unranked_plot
...
#make facet titles italic and add n's
# paste(unique(.$Species_full)," (n = ", length(unique(.$ID)), ")", sep ="")
#arrange bats from highest mean response to lowestX
#collapse 3 into 2 (use a diff response scale)
#respace dots
...
# ggtitle(paste(unique(.$Species_full)," (n = ", length(unique(.$ID)), ")", sep ="") )
