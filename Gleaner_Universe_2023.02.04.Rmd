...
#use relevent working directory
gleaner_universe_r_8_15_22 <- read_excel("~/Dropbox/GLEANER UNIVERSE STUDY/data sheets/gleaner_universe_r_08.15.22.xlsx", 
    na = "NA")
...
ID: Bat ID   
Sex = (duh)   
Date Set = Which suite of calls was used (Set 1- 4)   
...
Then a bunch of callparameters:    
call_duration   
"peak_freq"   
"min_freq"   
"max_freq"   
...
p_avg_batspecies<- 
      ...
      ggplot( aes(x=reorder(Species_short, -Response), y = Response), color = "black") + #the "-" orders it high to low
      geom_violin() +
...
#ggsave("Mean bat species responses to sounds by species.jpg", width = 10, height = 7)
...
p_avg_batspecies_calltype <-  p_avg_batspecies + 
      stat_summary(fun=mean, geom="point", shape=23, size=3, aes(fill= Call_type)) + 
...
p_avg_preyspecies <- 
      ...
      ggplot( aes(x=reorder(Playback, -Response), y = Response), color = "black") + #the "-" orders it high to low
      geom_violin() +
      # geom_point(size = 0.5, position = position_jitter(width = .3, height = 0), color = "lightgreen", alpha = 0.5) +
      coord_cartesian(ylim = c(0, 5)) + #standardize y axis
...
#ggsave("Mean responses to sounds- gray equals any bat landed.pdf")
```
...
Bat_plots = g_u %>%
    ...
    do(Bat_plots = p_avg_preyspecies_calltype %+% . + #do applies fn to current group. here it applies  plot to each Playback
      ...
      ggtitle(paste(unique(.$Species_full)," responses to playbacks (n = ", length(unique(.$ID)), ")", sep ="") ) #+ #makes title current group, n is number of bats          
             #ggsave(filename = paste(unique(.$Species_full),"responses_to_playbacks", ".pdf", sep = "")) 
       ) 
Bat_plots$Bat_plots
...
p_emmeans_batspecies<- 
      ggplot(as.data.frame(sp_em), aes(x=reorder(Species_full, -emmean), ymin=asymp.LCL,ymax=asymp.UCL, color = "darkgreen")) +
      geom_linerange(size = 4, color= "darkolivegreen4") +
      geom_point(aes(Species_full, emmean, size = 1), color = "darkolivegreen3") +
...
p_emmeans_call_type<- ggplot(cld_call_type, aes(x = reorder(Call_type, -emmean),ymin=asymp.LCL,ymax=asymp.UCL)) + 
    ...
    xlab("Call Category") +
    ylab("Interest score estimated marginal means")
...
#INCIDENTAL SOUNDS incidenteal plot 1
