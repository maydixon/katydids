...
#Fix misspelled "Lampronycteris brachyotis" and "Lonchorhina aurita"
g_u <- g_u %>%
      mutate(Species_full = replace(Species_full, Species_full == "Lampronycteris brachiotis", "Lampronycteris brachyotis" )) %>%
      mutate(Species_full = replace(Species_full, Species_full == "Lonchorina aurita", "Lonchorhina aurita"))
...
g_u <- g_u %>% mutate(Species_short = case_when(
      ...
      Species_full == "Lampronycteris brachyotis" ~ "La. brachyotis",
      Species_full == "Lonchorhina aurita" ~ "Lon. aurita",
...
n_species <-g_u %>% group_by(Species_short) %>%
      summarise(N_total = n_distinct(ID, na.rm=TRUE),
                N_Female = length(unique(ID[Sex == 'F'])),
                N_Male = length(unique(ID[Sex == 'M'])), 
                N_approach = length(Response[Response ==4] ),
                N_land = length(Response[Response ==5] )
                ) %>%
...
#summarize # approaches and lands
n_approachland <-g_u %>% group_by(Species_short, Playback) %>%
      summarise(N_total = n_distinct(ID, na.rm=TRUE),
                N_Female = length(unique(ID[Sex == 'F'])),
                N_Male = length(unique(ID[Sex == 'M'])), 
                N_approach = length(Response[Response ==4] ),
      ...
      filter(N_approach!=0 | N_land!=0, keep = TRUE)  %>%
      arrange(desc(N_total)) %>% 
      ungroup() %>%
      relocate(Playback, .after = N_land)
     
n_approachland

#add back in  the species that were dropped bc no approaches or lands

n_approachland_all <- rbind( n_approachland, (filter(n_species,  N_approach==0 & N_land==0, keep = TRUE) %>% mutate(Playback = NA)))%>%
      arrange(desc(N_total))%>%
      relocate(Playback, .before = N_approach) %>%
      rename(Species = Species_short,
             N = N_total,
             "N females" = N_Female,
             "N males" = N_Male,
             Approaches = N_approach, 
             Lands = N_land) %>%
      mutate(Approaches = paste(Playback, " (n=", Approaches, ")", sep = "" ),
             Lands = paste(Playback, " (n=", Lands, ")", sep = "" )) %>%
      mutate( Approaches = ifelse(grepl("0", Approaches), "NA", Approaches), #turn 0s to NAs
              Lands = ifelse(grepl("0", Lands), "NA", Lands)) %>%
      select(!Playback)

n_approachland_all     

#write.csv(n_approachland_all, "N_approachland_all.csv")
...
# ranked = sort each x axis from high to low response? 
# Boots = add bootstrapped values?
#N_min_mean /int: if adding boots, what should the minimum sample size be for adding the boot means, boot 95% confidence intervals?

facet_plot_bats <- function(d=d,
                            ranked= FALSE,
                            Boots = FALSE,
                            N_min_mean = NULL,
                            N_min_int = NULL,
...
# delete bootstrapped values below minimum sample size for plotting (could also do by n per each playback not total bat n)


Boot_sums <- Boot_sums %>%
      mutate(low = ifelse(n>=N_min_int, low, NA )) %>% 
      mutate(high = ifelse(n>=N_min_int, high, NA )) %>%
      mutate(mean = ifelse(n>=N_min_mean, mean, NA )) 
 ...
 if(ranked){
    plot <-   
     ...
     plot = plot +    geom_point(data = Boot_sums,
      ...
      # #make pdfs of plots (85, 129, or 177 mm width to fit in 1-3 columns, up to 238 mm height)  
       # ggsave("testplot.jpg",  width = 177, height =800, units = "mm")
                 plot = plot +   ggsave(filename = paste("faceted_", ifelse(ranked,"ranked_", "unranked_"), ifelse(Boots,"CIs_", "no_CIs_"), title, "responses_to_playbacks", ".jpg", sep = ""), device = "jpeg", width = Width, height =Height, units = "mm" )
...
unranked_plot_boot <- facet_plot_bats(d = g_u, ranked = FALSE, Boots = TRUE, N_min_int =10, N_min_mean = 10, title = "all_bats_")
# 
...
###### IGNORE ABOVE ################
```
Things of logical way to divide the plots up. If fitting into 2 pages, 1 col each

Bats that landed on something or are known or likely  gleaners:
"Tra. cirrhosus"
   "Lop. silvicolum"
  "M. hirsuta"      
 "M. microtis"  
...
Reasonably split plots:2 groups
```{r}
# probable gleaners
      g_u %>%
      filter(Species_full %in% c("Trachops cirrhosus",
       ...
       mutate(Species_full = factor(Species_full,
                                    c("Trachops cirrhosus",
                                "Lophostoma silvicolum",
                                "Lophostoma brasiliense",
                                "Micronycteris hirsuta",
                                "Micronycteris microtis",
                                "Micronycteris schmidtorum",
                                "Tonatia saurophila" ), ordered = TRUE )) %>%
      facet_plot_bats(d = ., ranked = FALSE, Boots = FALSE, Height = 238, title ="confirmed_gleaners_")
 #up to 238 mm height) 
      

#unknown and omnivores
g_u %>%
      filter(Species_full %in% c("Phyllostomus hastatus",
       ...
       mutate(Species_full = factor(Species_full,
                                  ...
                                  "Lonchorhina aurita",
                                  "Glyphonycteris daviesi",
                                  "Artibeus jamaicensis"), ordered = TRUE )) %>%
facet_plot_bats(d = ., ranked = FALSE, Boots = FALSE, legend_position ="none", Height = 238, title ="OtherOrUnknown_")


#change function so I can reorder the facets from the fn #test whether this even works
```
  
  
3 groups
Bats that landed on something:
"Tra. cirrhosus" ,
"Lop. silvicolum",
  "M. hirsuta"    ,  
 "M. microtis"  ,
...
plot with species with >10 bats 
```{r}

#pull spp with 10 or more individuals
keep <- g_u %>% group_by(Species_full) %>% summarize(n = length(unique(ID))) %>% filter(n>=10) %>% pull(Species_full) 

top_bats <-  g_u %>%
       filter( Species_full %in% keep)  %>%
       facet_plot_bats(d = ., ranked = FALSE, Boots = TRUE, N_min_int =10, N_min_mean = 10, Height = 238, title ="over_10n_")
top_bats        
...
Individual plots, one bat per plot, ranked and unranked x axis (same basic plots as above)
```{r}
...
## Add plots with 1 plot per playback  (could just generalize last fn)
...
p_avg_batspecies<- 
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor =  element_blank(),
       ...
       legend.position = "none", axis.text.x = element_text(angle = 63, hjust=1, face = "italic"), axis.text =                   element_text(size = 11, color = "black"), axis.title = element_text(size = 11),  )  + #gets rid of                    background
     # dark_theme_noline +
...
#add if I want mean added to each plot
# p_avg_batspecies <-  p_avg_batspecies + 
...
Playback_plots <- g_u %>%
    group_by(Playback) %>%
    do(Playback_plots = p_avg_batspecies %+% . + #do applies fn to current group. here it applies  plot to each Playback
             #facet_wrap(~Playback) + #can use if you want facet labels instead of/with ggtitle
              ggtitle(paste("Bat responses to ", unique(.$Playback), " (", unique(.$Call_type) , ")", sep = "")) + #makes title current group
             ggsave(filename = paste("bat_mean_responses_to_", unique(.$Playback), " (", unique(.$Call_type) , ")", ".jpg", sep = ""),
                    width = 177,
                    height = 100,
...
Playback_plots <-Playback_plots$Playback_plots

#name playback plots with Playback and call type
distinct(g_u, Playback, .keep_all = TRUE) %>%
      select(Playback, Call_type) %>%
      arrange(Playback) %>%
      mutate(Play_type = paste(Playback, Call_type, sep = "_")) %>%
      pull(Play_type) -> names(Playback_plots)
 
 Playback_plots   

#print plots (right now they're grouped by call_type)
Arrange_playbacks <- function(Group){
ggpubr::ggarrange(plotlist = Playback_plots[grepl(Group, names(Playback_plots))],  ncol = 1) %>%
            ggsave(filename =paste("Playback_plots_", Group, ".jpg", sep =""),  width = 177, units = "mm",
                   height = (90 *length(Playback_plots[grepl(Group, names(Playback_plots))]))
                   ) #scale height by # of plots
}


lapply(unique(g_u$Call_type), Arrange_playbacks)
...
- this is going to be pulled by the different # of bats of different species. could deal with this in a few ways:
   - weight the average per bat species (account for there being more bats of some species than others. = (average response/ spp1) + (avg response/ spp2)…/ n species,
...
```{r mean responses to call categories, {r, eval=F, echo=F}
...
g_u_p_LaB <- g_u_p_LaB + G3 +C3 + TM4 + Xlab2 + Ylab2  +cLaB + #all parameters for general data
      ...
      geom_point(data =Resp_avg[Resp_avg$Species_full =="Lampronycteris brachyotis",], mapping=aes(x = Playback, y = Resp_average), color = "black", shape=17, size = 4 ) #parameters for the averages
...
g_u_p_Loa <- g_u_p_Loa + G3 +C3 + TM3 + Xlab2 + Ylab2  +cLA + #all parameters for general data
      ...
      geom_point(data =Resp_avg[Resp_avg$Species_full =="Lonchorhina aurita",], mapping=aes(x = Playback, y = Resp_average), color = "black", shape=17, size = 4 ) #parameters for the averages
