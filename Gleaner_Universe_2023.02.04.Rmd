...
p_avg_batspecies<- 
      ...
      ggplot( aes(x=reorder(Species_short, -Response), y = Response), color = "white") + #the "-" orders it high to low color = "black" or "white" for dark or light mode
      geom_violin() +
...
Playback_plots = g_u %>%
    ...
    do(Playback_plots = p_avg_batspecies_calltype %+% . + #do applies fn to current group. here it applies  plot to each Playback
             ...
             ggsave(filename = paste("bat_mean_responses_to_",unique(.$Playback),"_dark.pdf", sep = "")) 
       ) 
...
Bat_plots = g_u %>%
    ...
    do(Bat_plots = p_avg_preyspecies %+% . + #do applies fn to current group. here it applies  plot to each Playback
